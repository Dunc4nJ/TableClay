...
// Featured collections configuration with their hero images and descriptions
...
const CollectionShowcases = ({ collections }: CollectionShowcasesProps) => {
  ...
  const featuredCollections = FEATURED_COLLECTIONS.map((featured) => {
    const collection = collections.find((c) => c.handle === featured.handle)
    return collection
      ? {
          ...collection,
